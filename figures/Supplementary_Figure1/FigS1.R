#Fig.S1 taxonomic statistic of 19,251 SGBs.
library(tidyverse)
library(readxl)
...
taxon_comp <- function(infile, outfile, top_num){
  ...
  ggsave(outfile, SGB_taxon, width = 3, height = 4)
}

taxon_comp("0.data/Tibetan.bac.count.tsv", "1.result/figS1A.Tibetan_bac_taxa.nolegend.pdf", 5)
taxon_comp("0.data/Tibetan.ar.count.tsv", "1.result/figS1B.Tibetan_ar_taxa.nolegend.pdf", 3)
