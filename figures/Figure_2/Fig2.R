...
dist.p <- ggplot(dset.c) + 
  ...
  geom_point(aes(x = 0.05, 1705), color = "#13b6bb") +
  geom_point(aes(x = 0.05, 334), color = "#f56b64") +
  geom_point(aes(x = 0.15, 6230), color = "#13b6bb") +
  geom_point(aes(x = 0.15, 1331), color = "#f56b64") +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 0.2)) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 12000), breaks = seq(0,12000,2000)) +
  theme_classic(base_family = "Helvetica") +
  theme(legend.position = c(0,1), legend.justification = c(0, 1))
...
dset.d <- read_csv("0.data/dset.fig2D.csv") %>% 
  filter(improve_rate > 10 & QTP_freq > 5)
